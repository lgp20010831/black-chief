...
public class SingletonClazzTree {


    private final Map<Class<?>, Class<?>> cache = new HashMap<>();

    private final Map<Class<?>, List<Class<?>>> mappingKeyCollection = new HashMap<>();

    public Map<Class<?>, List<Class<?>>> getMappingKeyCollection() {
    ...
    public void register(Class<?> targetClass, Class<?> key){
        List<Class<?>> saveList = mappingKeyCollection.computeIfAbsent(targetClass, k -> new ArrayList<>());
        if (!saveList.contains(key)){
            saveList.add(key);
        }
    }
    public <Q> Class<?> querySinletonKey(@NotNull Class<Q> condition){
        List<Class<?>> mappingKeys = getMappingKeys(condition);
        return mappingKeys.isEmpty() ? null : mappingKeys.get(0);
    }

    public <Q> List<Class<?>> getMappingKeys(@NotNull Class<Q> condition){
        List<Class<?>> list = mappingKeyCollection.get(condition);
        if (list == null){
