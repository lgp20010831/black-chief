...
public class UpdateSessionExecutor implements PointSessionExecutor {
    ...
    public ExecuteBody doExecute(ExecutePacket ep) {
        ...
        for (GlobalSQLRunningListener listener : configuration.getRunningListener()) {
            sql = listener.postUpdateSql(configuration, sql, valueGroupList);
        }
        return session.pipelineUpdates(sql, valueGroupList);
    }
}
