...
import com.black.core.sql.annotation.AutomaticallyUpdateOrInsert;
...
@SQLListener
public class BlendUpdateOrInsertListener implements GlobalSQLRunningListener {


    private final ThreadLocal<Object> lock = new ThreadLocal<>();

    private final Map<Method, BlendUpdateOrAddConfig> conifgCache = new ConcurrentHashMap<>();

    private final BlendUpdateMasterProcessor updateMasterProcessor;

    private final BlendInsertMasterProcessor insertMasterProcessor;

    public BlendUpdateOrInsertListener() {
        updateMasterProcessor = new BlendUpdateMasterProcessor();
        insertMasterProcessor = new BlendInsertMasterProcessor();
    }
    ...
    public boolean interceptBatchs(String sql, SqlValueGroup sqlValueGroup) {
        ...
        try {
            ...
            if (!conifgCache.containsKey(methodWrapper.getMethod())){
                if (!methodWrapper.hasAnnotation(AutomaticallyUpdateOrInsert.class) ||
                        !(configuration.getMethodType() == SQLMethodType.INSERT ||
                                configuration.getMethodType() == SQLMethodType.UPDATE)){
                    return false;
                }
                AutomaticallyUpdateOrInsert annotation = methodWrapper.getAnnotation(AutomaticallyUpdateOrInsert.class);
                BlendUpdateOrAddConfig config = AnnotationUtils.loadAttribute(annotation, new BlendUpdateOrAddConfig(configuration.getGlobalSQLConfiguration(), methodWrapper));
                config.setCw(configuration.getCw());
                SQLMethodType updateMethod = configuration.getMethodType() == SQLMethodType.UPDATE ? SQLMethodType.INSERT : SQLMethodType.UPDATE;
                config.setMethodType(updateMethod);
                ...
                ConfigurationTreatment.treatmentConfig(config, wrapper);
                config.setMethodType(updateMethod == SQLMethodType.UPDATE ? SQLMethodType.INSERT : SQLMethodType.UPDATE);
                conifgCache.put(methodWrapper.getMethod(), config);
            }
            boolean intercept = false;
            BlendUpdateOrAddConfig conifg = conifgCache.get(methodWrapper.getMethod());
            try {
    ...
    public void intercptCallback(String sql, SqlValueGroup sqlValueGroup, ExecuteBody executeBody) {
        ...
        BlendUpdateOrAddConfig conifg = conifgCache.get(mw.getMethod());
