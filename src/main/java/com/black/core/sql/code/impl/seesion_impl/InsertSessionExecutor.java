...
public class InsertSessionExecutor implements PointSessionExecutor {
    ...
    public ExecuteBody doExecute(ExecutePacket ep) {
        ...
        for (GlobalSQLRunningListener listener : configuration.getRunningListener()) {
            sql = listener.postInsertSql(configuration, sql, valueGroupList);
        }
        return session.pipelineInserts(sql, valueGroupList);
    }
}
