...
public class InSeqParser extends AbstractSqlSeqParser {
    @Override
    public boolean support(String seq, OperationType type) {
        return type == OperationType.SELECT && seq.contains("in") && StringUtils.isIndependent(seq, "in");
    }

    @Override
    public boolean queryParse(String seq, SqlOutStatement statement, Map<String, Object> argMap, TableMetadata metadata) {
        String[] eqkv = StringUtils.split(seq, "in", 2, "error parse seq: " + seq);
        ...
        if (saveColumn(metadata, columnName)){
            ...
            String sql = statement.getColumnName(columnName) + " in " + val;
