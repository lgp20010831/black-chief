...
public class BlendUpdateMasterProcessor extends AbstractBlendProcessor {
    ...
    boolean updateExist(String sql, SqlValueGroup sqlValueGroup,
        ...
        if (configuration instanceof AppearanceConfiguration) {
            ...
            //拿到主表与子表的关系
            Set<String> whenAppearanceSingleMapping = conifg.getWhenAppearanceSingleMapping();
            //判断是否为一对多的关系
            boolean single = whenAppearanceSingleMapping.contains(appearanceConfiguration.getTableName());
            if (!single) {
                //如果是一对多的关系
                Map<String, BlendObjects> objectsMap = BlendsManager.getAndParse(mw);
                BlendObjects update = objectsMap.get("update");
                Set<String> defaultConditions = new HashSet<>();
                String primaryName = appearanceConfiguration.getPrimaryName();
                if (primaryName != null) {
                    defaultConditions.add(primaryName);
                }

                if (update != null) {
                    BlendObjects child = update.getChild(appearanceConfiguration.getTableName());
                    if (child != null) {
                        defaultConditions.addAll(child.getAttributes());
                    }
                }

                //获取到查询的关键字段, 然后判断当前查询条件中是否存在这些字段, 存在则走正常流程
                //否则直接拦截
                boolean intercept = true;
                for (String defaultCondition : defaultConditions) {
                    if (sqlValueGroup.findSingle(defaultCondition, OperationType.SELECT) != null) {
                        intercept = false;
                        break;
                    }
                }
    ...
    void processorUpdate(String sql, SqlValueGroup sqlValueGroup,
        ...
        for (String columnName : tableMetadata.getColumnNameSet()) {
            ...
            if (value != null) {
                Object realval = value.getValue();
                if (realval != null){
                    String val = SQLUtils.getString(realval);
