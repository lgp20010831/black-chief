...
public class NotInSeqParser extends AbstractSqlSeqParser {
    @Override
    public boolean support(String seq, OperationType type) {
        return type == OperationType.SELECT && seq.contains("not in") && !seq.contains("=");
    }


    @Override
    public boolean queryParse(String seq, SqlOutStatement statement, Map<String, Object> argMap, TableMetadata metadata) {
        String[] eqkv = StringUtils.split(seq, "not in", 2, "error parse seq: " + seq);
        ...
        if (saveColumn(metadata, columnName)){
            ...
            String sql = statement.getColumnName(columnName) + " not in " + val;
