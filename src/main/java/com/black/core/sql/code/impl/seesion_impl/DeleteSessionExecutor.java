...
public class DeleteSessionExecutor implements PointSessionExecutor {

    @Override
    public boolean support(Configuration configuration) {
        return configuration.getMethodType() == SQLMethodType.DELETE;
    }
    ...
    public ExecuteBody doExecute(ExecutePacket ep) {
        ...
        for (GlobalSQLRunningListener listener : configuration.getRunningListener()) {
            sql = listener.postDeleteSql(configuration, sql, valueGroupList);
        }
        return session.pipelineDeletes(sql, valueGroupList);
    }
}
